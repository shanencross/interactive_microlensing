...
function init() {
  ...
  updatePlotScaleAndRange(dayWidthDefault, magnifHeightDefault, xAxisInitialDayDefault, yAxisInitialMagnifDefault);
  updateCurveData();
...
function initListeners() {
  ...
  if (updateOnSliderMovement == true) {
    ...
    u0slider.addEventListener("input", function () {
      updateParam("u0");
    }, false);
    Ml1slider.addEventListener("input", function () {
      updateParam("Ml1");
    }, false);
    Ml2slider.addEventListener("input", function () {
      updateParam("Ml2");
    }, false);
    DsSlider.addEventListener("input", function () {
      updateParam("Ds");
    }, false);
    thetaYslider.addEventListener("input", function () {
      updateParam("thetaY");
    }, false);
    ...
    inclineSlider.addEventListener("input", function () {
      updateParam("incline");
    }, false);
    lensSepSlider.addEventListener("input", function () {
      updateParam("lensSep");
    }, false);
  }
  ...
  if (updateOnSliderRelease === true) {
    tEslider.addEventListener("change", function () {
      updateParam("tE");
    }, false);
    u0slider.addEventListener("change", function () {
      updateParam("u0");
    }, false);
    Ml1slider.addEventListener("change", function () {
      updateParam("Ml1");
    }, false);
    Ml2slider.addEventListener("change", function () {
      updateParam("Ml2");
    }, false);
    DsSlider.addEventListener("change", function () {
      updateParam("Ds");
    }, false);
    thetaYslider.addEventListener("change", function () {
      updateParam("thetaY");
    }, false);
    DlSlider.addEventListener("change", function () {
      updateParam("Dl");
    }, false);
    t0slider.addEventListener("change", function () {
      updateParam("t0");
    }, false);
    muSlider.addEventListener("change", function () {
      updateParam("mu");
    }, false);
    inclineSlider.addEventListener("change", function () {
      updateParam("incline");
    }, false);
    lensSepSlider.addEventListener("change", function () {
      updateParam("lensSep");
    }, false);
  ...
  fixU0checkbox.addEventListener("change", function () {
    fixU0 = fixU0checkbox.checked;
    console.log("fixU0: " + fixU0);
  }, false);
  // debug plot range/scale and reset buttons
  xLeftButton.addEventListener("click", function () {
    updateGraph("xLeft");
  }, false);
  xRightButton.addEventListener("click", function () {
    updateGraph("xRight");
  }, false);
  yUpButton.addEventListener("click", function () {
    updateGraph("yUp");
  }, false);
  yDownButton.addEventListener("click", function () {
    updateGraph("yDown");
  }, false);

  xZoomInButton.addEventListener("click", function () {
    updateGraph("xZoomIn");
  }, false);
  xZoomOutButton.addEventListener("click", function () {
    updateGraph("xZoomOut");
  }, false);
  yZoomInButton.addEventListener("click", function () {
    updateGraph("yZoomIn");
  }, false);
  yZoomOutButton.addEventListener("click", function () {
    updateGraph("yZoomOut");
  }, false);
...
function initParams() {
  ...
  updateDerivedQuantities(true);
}

/** updateDerivedQuantities */
function updateDerivedQuantities() {
  var initializing = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;

  updateDrel();
  updateThetaE();
  if (fixU0 === false || initializing === true) updateU0();else updateThetaY();
...
/** calculateThetaE */
function calculateThetaE() {
  var get_mas = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
  var useBinaryMass = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
  var lensToUse = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 1;
  ...
  */
  ...
  if (get_mas === true) thetaEresult = thetaEresult / masToRad;
...
/** updateSliderReadout */
function updateSliderReadout(slider, readout) {
  var sliderName = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : "";
  ...
  if (sliderName === "Ml1" || sliderName === "Ml2") fixedDecimalPlace = 6;else if (sliderName === "Ds" || sliderName === "Dl" || sliderName === "mu" || sliderName === "incline") fixedDecimalPlace = 2;else if (sliderName === "t0") {
    fixedDecimalPlace = 1;
...
function updateGraph(shift) {
  ...
  if (shift === undefined) return;else if (shift === "xLeft") {
...
function updateGridRange(xStep, yStep) {
  ...
  // increment
  if (xAxisInitialDay % xGridStep === 0) xGridInitial = xAxisInitialDay;else xGridInitial = xGridStep * (Math.floor(xAxisInitialDay / xGridStep) + 1);

  // same rounding for final grid line placement
  if (xAxisFinalDay % xGridStep === 0) xGridFinal = xAxisFinalDay;else xGridFinal = xGridStep * Math.floor(xAxisFinalDay / xGridStep);

  // same rounding for initial y grid line placement
  if (yAxisInitialMagnif % yGridStep === 0) yGridInitial = yAxisInitialMagnif;else yGridInitial = yGridStep * (Math.floor(Math.floor(yAxisInitialMagnif) / yGridStep) + 1);

  // same rounding for final y grid line placement
  if (yAxisFinalMagnif % yGridStep === 0) yGridFinal = yAxisFinalMagnif;else yGridFinal = yGridStep * Math.floor(yAxisFinalMagnif / yGridStep);
}
...
function updatePlotScaleAndRange(width, height, xInit, yInit) {
  ...
  // plot scale
  if (width !== undefined) dayWidth = width;
  if (height !== undefined) magnifHeight = height;
  xPixelScale = graphWidth / dayWidth; // pixels per day
  yPixelScale = graphHeight / magnifHeight; // pixels per magnif unit
  if (xInit !== undefined) xAxisInitialDay = xInit;
  if (yInit !== undefined) yAxisInitialMagnif = yInit;
...
function plotLightcurve() {
  var tDayFinal = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : xAxisFinalDay;
  var inputData = arguments[1];
  ...
  plotLightcurveAlone(xAxisFinalDay, inputData, true);
  // draw lightcurve up to the time argument as solid line
  plotLightcurveAlone(tDayFinal, inputData, false);
...
/** plotLightcurveAlone */
function plotLightcurveAlone() {
  var tDayFinal = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : xAxisFinalDay;
  var inputData = arguments[1];
  var dashedCurve = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;

  // draw a single lightcurve (dashed or solid) up to a given time

  var tDay, magnif;
  if (inputData !== undefined) {
    var curveData = inputData;
  }
  // no input parameter given
      ...
      if (lightcurveData !== null) {
        ...
        var curveData = lightcurveData;
      }
      // module lightcurve variable not initialized yet
      else {
          // initialize module variable
          updateCurveData();
          var curveData = lightcurveData; // use newly initialized module variable in function
        }
    }
  ...
  if (dashedCurve === true) context.setLineDash([]); // turn off dashed lines
  context.restore();
}
...
function updateCurveData() {
  ...
  */
  ...
  var binaryCaclulationResults = bin_len_faster.plot_binary(GM1, GM2, D, cof1, cof2, minXLM, maxXLM, NPN, NR, DR);
  ...
  if (prerenderCurves === true && lensPlaneModule !== undefined && lensPlaneModule.initialized === true) {
    lensPlaneModule.renderCurves();
  }

  var updateLensedImages = true;
  if (updateLensedImages === true && lensPlaneModule !== undefined && lensPlaneModule.initialized === true) {
...
function getMagnifFromU(u) {
  ...
  // unitless
  var magnif = magnifNumerator / magnifDenominator;
  return magnif;
}
...
module.exports = {
  ...
  get updateOnSliderMovementFlag() {
    return updateOnSliderMovementFlag;
  },
  get updateOnSliderReleaseFlag() {
    return updateOnSliderReleaseFlag;
  },
  ...
  get xAxisInitialDay() {
    return xAxisInitialDay;
  },
  get xAxisFinalDay() {
    return xAxisFinalDay;
  },
  ...
  get imageParities() {
    if (lightcurveData !== null && lightcurveData !== undefined) return lightcurveData.imageParities;
  },

  // normalized (over thetaE) positions of the (five or three) lensed images
  get imagesNormalizedPos() {
    if (lightcurveData !== null && lightcurveData !== undefined) return lightcurveData.imagesNormalizedPos;
  },

  // get caustic and critical curve data points,
  // normalized in units of (binary) thetaE
  get causticNormalized() {
    if (lightcurveData !== null && lightcurveData !== undefined) return lightcurveData.causticNormalized;
  },

  get critNormalized() {
    if (lightcurveData !== null && lightcurveData !== undefined) return lightcurveData.critNormalized;
  },

  get times() {
    if (lightcurveData !== null && lightcurveData !== undefined) return lightcurveData.times;
  },
