...
function init() {
  ...
  if (animationStep >= 0) time = minTime;else time = maxTime;
...
function updateMinAndMaxTimes(min, max) {
  ...
  if (min === undefined) min = eventModule.xAxisInitialDay;

  if (max === undefined) max = eventModule.xAxisFinalDay;
...
function initListeners() {
  ...
  stepForwardButton.addEventListener("click", function () {
    updatePlayback("stepForward");
  }, false);
  timeResetButton.addEventListener("click", function () {
...
function run() {
  if (running === true) {
    ...
    if (time >= maxTime || almostEquals(time, maxTime) === true || time <= minTime || almostEquals(time, minTime) === true) {
      updatePlayback("Pause");
    }
  }
}
...
function updateTime(newTime) {
  ...
  if (newTimeOverMax === true && newTimeUnderMin === true) updatePlayback("Pause");
...
function updatePlayback() {
  var command = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : "play";
  var updateFrame = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;
  ...
  if (command === "stepBack") {
    ...
    if (time > minTime && almostEquals(time, minTime) === false) {
      ...
      if (updateFrame === true) animateFrame();
    }
